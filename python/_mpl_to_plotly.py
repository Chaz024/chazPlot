...
def mixing_efficiency_map(Y, window_cells=15, y_min_presence=0.02):
    """
    Indice de melange de Danckwerts calcule LOCALEMENT (fenetre glissante
    de window_cells x window_cells), au lieu d'une moyenne sur toute une
    colonne verticale. Donne une CARTE 2D montrant OU le melange a
    vraiment lieu -- pas juste une valeur moyenne qui masque le detail
    spatial. Purement post-traitement (n'utilise que Y deja resolu par
    le solveur, aucune modification du solveur necessaire).

    0 = zone localement segregee (le fluide n'est pas mélangé ici)
    1 = zone localement bien melangee (homogene a l'echelle de la fenetre)
    NaN = zone ou aucun des deux fluides n'est presence significative
    """
    Ybar = uniform_filter(Y, size=window_cells, mode='nearest')
    Y2bar = uniform_filter(Y**2, size=window_cells, mode='nearest')
    var = np.maximum(Y2bar - Ybar**2, 0.0)
    var_max = Ybar * (1.0 - Ybar)
    with np.errstate(divide='ignore', invalid='ignore'):
        eta = 1.0 - var / np.maximum(var_max, 1e-9)
    meaningful = (Ybar > y_min_presence) & (Ybar < 1.0 - y_min_presence)
    eta = np.where(meaningful, eta, np.nan)
    return eta


def plot_mixing_map(d, window_mm=0.5, title_suffix=""):
    """Affiche la carte de melange local pour Y1 et Y2 -- fonction
    AUTONOME, ne lit que le .npz deja sauvegarde."""
    x, y = d['x']*1e3, d['y']*1e3
    dx_mm = x[1] - x[0]
    window_cells = max(3, int(round(window_mm / dx_mm)))

    Y1, Y2 = d['Y1'], d.get('Y2', np.zeros_like(d['Y1']))
    eta1 = mixing_efficiency_map(Y1, window_cells)
    eta2 = mixing_efficiency_map(Y2, window_cells) if int(d['n_slots']) > 1 else None

    X, Y = np.meshgrid(x, y, indexing='ij')
    n_panels = 2 if eta2 is not None else 1
    fig, axes = plt.subplots(n_panels, 1, figsize=(10, 4*n_panels), sharex=True)
    if n_panels == 1:
        axes = [axes]

    cmap = plt.cm.RdYlGn.copy()
    cmap.set_bad(color='white')  # zones NaN (rien a melanger) en blanc, neutre

    im0 = axes[0].pcolormesh(X, Y, eta1, shading='auto', cmap=cmap, vmin=0, vmax=1)
    axes[0].set_title(f'Melange local Jet 1 vs reste (fenetre={window_mm}mm) {title_suffix}')
    fig.colorbar(im0, ax=axes[0], label='eta local (0=segregue, 1=melange)')

    if eta2 is not None:
        im1 = axes[1].pcolormesh(X, Y, eta2, shading='auto', cmap=cmap, vmin=0, vmax=1)
        axes[1].set_title('Melange local Jet 2 vs reste')
        fig.colorbar(im1, ax=axes[1], label='eta local (0=segregue, 1=melange)')

    for ax in axes:
        for i in range(int(d['n_slots'])):
            ax.axvspan(d[f'slot{i}_xmin']*1e3, d[f'slot{i}_xmax']*1e3, color='cyan', alpha=0.4, zorder=5)
        ax.set_ylabel('y [mm]')
