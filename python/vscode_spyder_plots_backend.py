...
# Températures de 100 K à 1500 K
temperatures = list(range(100, 1600, 100))

# Enthalpie Sensible Expérimentale Brute (kJ/kg)
donnees_enthalpie = {
...
# Entropie Absolue Expérimentale Brute (J/kg.K)
donnees_entropie = {
    "Ar":  [3305, 3666, 3877, 4026, 4142, 4237, 4317, 4387, 4448, 4503, 4553, 4598, 4640, 4678, 4714],
    "CO":  [5840, 6562, 6982, 7278, 7513, 7710, 7880, 8030, 8166, 8289, 8403, 8508, 8607, 8699, 8786],
    "N2":  [5718, 6424, 6843, 7143, 7377, 7572, 7739, 7887, 8021, 8142, 8254, 8358, 8455, 8546, 8631],
    "CO2": [np.nan, np.nan, 4860, 5122, 5344, 5539, 5713, 5868, 6010, 6140, 6261, 6373, 6478, 6576, 6669],
    "NO":  [5901, 6623, 7030, 7317, 7541, 7728, 7890, 8035, 8166, 8285, 8395, 8497, 8592, 8681, 8765],
    "O2":  [5411, 6047, 6417, 6684, 6898, 7079, 7236, 7376, 7503, 7618, 7723, 7820, 7910, 7995, 8074],
    "H2":  [50635, 59527, 64892, 69059, 72293, 74947, 77195, 79157, 80896, 82468, 83913, 85249, 86498, 87671, 88780],
    "H2O": [np.nan, np.nan, 1049, 1104, 1147, 1184, 1216, 1245, 1271, 1295, 1317, 1338, 1358, 1377, 1395],
    "H":   [np.nan, np.nan, 113941, 119874, 124476, 128235, 131414, 134168, 136597, 138770, 140735, 142530, 144181, 145709, 147132],
    "O":   [np.nan, np.nan, 10075, 10465, 10762, 11004, 11207, 11383, 11537, 11675, 11799, 11913, 12017, 12114, 12204],
    "N":   [np.nan, np.nan, 10954, 11381, 11712, 11983, 12211, 12410, 12584, 12741, 12882, 13011, 13130, 13240, 13342],
    "OH":  [np.nan, np.nan, 10771, 11254, 11627, 11928, 12181, 12400, 12593, 12766, 12922, 13064, 13194, 13315, 13427]
}

def creer_et_sauvegarder_csv():
    # Création des DataFrames
    df_h = pd.DataFrame(donnees_enthalpie, index=temperatures).T
    df_s = pd.DataFrame(donnees_entropie, index=temperatures).T
    
    # Nommage de l'index pour que l'en-tête soit clair
    df_h.index.name = "Espece"
    df_s.index.name = "Espece"
    
    # Exportation (séparateur point-virgule pour une ouverture directe sous Excel)
    df_h.to_csv("enthalpie_janaf.csv", sep=";")
    df_s.to_csv("entropie_janaf.csv", sep=";")
    
    print("Succès : Les fichiers 'enthalpie_janaf.csv' et 'entropie_janaf.csv' ont été générés.")
